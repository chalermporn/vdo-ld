...
`ensure_tools` (โหลด yt-dlp/ffmpeg ถ้าขาด) → `download` (yt-dlp `bv*+ba/b` → merge mp4 `-c copy` ไม่ re-encode) → `verify` (ffprobe ดึง w/h/codec/size) → `file_into` (ย้ายเข้า `~/VDO/<หมวด>/<title>.<ext>`) → `index_record` (บันทึก provenance ลง SQLite, best-effort).

### Index / provenance DB (`index_record`, `search_index`, `backfill_index`)
เก็บ source URL + metadata ทุกครั้งที่โหลด ลง DB กลาง `<vdo_root>/.vdo-dl/index.db` เพื่อค้นย้อนหลัง. **shell out ไป `sqlite3` ที่มากับ OS** (เหมือนวิธีเรียก yt-dlp/ffmpeg) — ไม่ดึง Rust crate จึง **คง zero-dep ของ core**. กฎที่ต้องรักษา:
- **best-effort**: เครื่องไม่มี `sqlite3` → `index_record` คืน `Ok(())` เงียบ ไม่ให้การโหลดพัง. หา tool ผ่าน `find_tool("sqlite3", "-version")`.
- **กัน SQL injection ด้วย `sql_quote`** (double single-quote) — ห้ามต่อค่าผู้ใช้/ชื่อไฟล์เข้า SQL ตรงๆ. `dest_path` UNIQUE → upsert (`ON CONFLICT DO UPDATE`) index ซ้ำได้ไม่เพิ่มแถว. `SCHEMA` นำหน้าทุก statement (idempotent) ไม่ต้อง init แยก.
- provenance ดึง **ฟรีตอนโหลด** ผ่าน `--print after_move:DONE:::…` (id/title/webpage_url/uploader/upload_date/duration/extractor/filepath) แนบมากับ `DlEvent::ItemDone { meta }` + `DownloadedItem.meta` — **ไม่เรียก yt-dlp ซ้ำ**. แก้ template ต้องแก้จำนวนช่อง `splitn` ใน parser ของ `download` ให้ตรง (ตอนนี้ 9).
- `backfill_index` สแกนไฟล์เก่าใน `~/VDO/` ที่ยังไม่อยู่ใน index. provenance กู้ได้แค่จากไฟล์ (หมวด=โฟลเดอร์ชั้นแรก, ชื่อ, ขนาด/ความละเอียด, เวลา=mtime) — **source URL ของเก่ากู้ไม่ได้** จึงเว้นว่าง.
...
cargo run -- search "คำค้น"              # ค้นประวัติที่เคยโหลด (title/ผู้ลง/หมวด/URL/แหล่ง)
cargo run -- backfill                    # สแกนไฟล์เก่าใน ~/VDO/ เข้า index (ของเก่าไม่มี URL)

cargo check                              # ตรวจเร็วทั้ง workspace
cargo clippy --all-targets               # lint

# ออก release (เลื่อน CHANGELOG + bump version + commit + tag + push ในคำสั่งเดียว)
# เขียนรายการใต้ ## [Unreleased] ใน CHANGELOG.md ก่อน แล้ว:
make release VERSION=0.1.4                # = ./scripts/release.sh 0.1.4
./scripts/release.sh 0.1.4 --dry-run     # ลองดูก่อน ไม่เขียน/ไม่ push
